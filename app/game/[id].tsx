...
const BLURBS: Record<string, string> = {
  stack: 'Tap to drop. Overhang gets sliced off. The tower narrows until it cannot.',
  reflex: 'Tap the instant the needle crosses the band. The band shrinks every round.',
  dodge: 'Three lanes, one of them is about to be wrong. Swipe before it is.',
};
...
export default function GameDetail() {
  ...
  const playable = ['stack', 'reflex', 'dodge'].includes(game.id);
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 28 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 14, padding: S.inset }}>
          ...
          <View style={{ flex: 1, justifyContent: 'center' }}>
            ...
            <Text style={[text('meta', { color: C.n700 }), { marginTop: 4 }]}>{BLURBS[game.id] ?? 'A quick one. Beat the number.'}</Text>
          </View>
        </View>
